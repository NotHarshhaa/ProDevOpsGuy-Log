...
const PostCard: React.FC<Props> = ({ post }) => {
  ...
  return (
    ...
    <span>
      ...
      >
        <div
          className="hidden sm:block relative my-auto h-0 w-[100%] pb-[100%] sm:col-span-1 cursor-pointer border border-gray-200 dark:border-gray-800 clay-1 rounded-3xl"
          ...
          {post.thumbnail ? (
            <Image
              ...
              className={`object-cover transition-all rounded-3xl ${
                isHoverTitle ? "brightness-100" : "brightness-90"
              }`}
              layout="fill"
              alt={post.title}
              loading="lazy"
        ...
        <div className="py-4 px-2 md:px-4 sm:col-span-4">
          ...
          <header className="flex flex-col justify-between md:flex-row md:items-baseline">
            <h2
              className={`text-xl md:text-[21px] cursor-pointer font-bold transition-all
                  ${
                    isHoverTitle
